...
import Refine.DoubleVector.DoubleVectorElement;
import Refine.EuclidianDistance;
...
public final class KDTree<VALUE> implements Iterable<DoubleVector> {
  ...
  private List<KDTreeNode> rangeInternal(DoubleVector lower, DoubleVector upper) {
    ...
    Deque<KDTreeNode> toVisit = new ArrayDeque<KDTreeNode>();
    toVisit.add(root);
  ...
  public List<VectorDistanceTuple<VALUE>> getNearestNeighbours(
    ...
    LimitedPriorityQueue<VectorDistanceTuple<VALUE>> queue = new LimitedPriorityQueue<VectorDistanceTuple<VALUE>>(
        k);
