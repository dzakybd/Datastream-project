...
import kdtree.KDTree.VectorDistanceTuple;
...
//    	//DoubleVector shift = numerator.divide(weightSum).subtract( v );
...
//      }
      ...
      for (VectorDistanceTuple<Integer> neighbour : neighbours) {
          if (neighbour.getDistance() < h) {
            double normDistance = neighbour.getDistance() / h;
            double weight = -gaussianGradient(normDistance);
            denominator += weight;
            numerator = numerator.add(neighbour.getVector().multiply(weight));
          }
        }
        if (denominator > 0d) {
